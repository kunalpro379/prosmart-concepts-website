...
export const fetchProductByCatSubcatId = async (catid: string, subcatid: string, productid: string): Promise<Product> => {
  const url = `${API_BASE_URL}/products/${catid}${subcatid}/${productid}`;
  const response = await fetch(url);
  const result: APIResponse<Product> = await response.json();
